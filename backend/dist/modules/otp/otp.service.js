...
class OtpService {
        ...
        if (customer) {
            ...
            if (recent) {
                ...
                if (elapsedMs < cooldownMs) {
                    throw new AppError_1.AppError(429, "Please wait before requesting another code");
                }
            }
            await otp_repository_1.default.invalidateActive(phone, OTP_PURPOSE);
            const code = Math.floor(100000 + Math.random() * 900000).toString();
            const codeHash = await bcrypt_1.default.hash(code, 10);
            const expiresAt = new Date(Date.now() + OTP_TTL_MINUTES * 60 * 1000);
            await otp_repository_1.default.create({
                phone,
                codeHash,
                purpose: OTP_PURPOSE,
                expiresAt,
    ...
    async verifyOtp(phone, code) {
        ...
        if (isExpired) {
            throw new AppError_1.AppError(400, "This code has expired. Please request a new one.");
        }
        if (otp.attempts >= MAX_VERIFY_ATTEMPTS) {
            throw new AppError_1.AppError(429, "Too many incorrect attempts. Please request a new code.");
        }
        const isValid = await bcrypt_1.default.compare(code, otp.codeHash);
        if (!isValid) {
            await otp_repository_1.default.incrementAttempts(otp.id);
            throw new AppError_1.AppError(400, "Incorrect code");
        }
        await otp_repository_1.default.markConsumed(otp.id);
        const customer = await customer_service_1.default.getByPhone(phone);
        if (!customer) {
            throw new AppError_1.AppError(404, "No application found for this phone number");
        }
