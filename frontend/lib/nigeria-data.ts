...
export const NIGERIA_STATE_OPTIONS = NIGERIA_STATES.map((s) => ({ value: s, label: s }));

// Reconstructed from general knowledge of Nigeria's 774 LGAs, not verified
// against a live source — spot-check states you expect real applicants
// from, and flag any corrections needed.
...
export function getLgaOptions(state: string): { value: string; label: string }[] {
  return (NIGERIA_LGAS[state] ?? []).map((lga) => ({ value: lga, label: lga }));
}

// No official enum exists for occupation anywhere in the schema (it's a
// plain string field) — this is a starting set for UI consistency, not a
// fixed business rule. Adjust freely; "Other" always falls back to free text.
export const OCCUPATION_OPTIONS = [
  ...
  { value: 'Artisan / Skilled Tradesperson', label: 'Artisan / Skilled Tradesperson' },
  { value: 'Teacher / Educator', label: 'Teacher / Educator' },
  { value: 'Healthcare Worker', label: 'Healthcare Worker' },
  { value: 'Driver / Transport Operator', label: 'Driver / Transport Operator' },
  { value: 'Tailor / Fashion Designer', label: 'Tailor / Fashion Designer' },
  { value: 'Hairdresser / Barber', label: 'Hairdresser / Barber' },
  { value: 'Caterer / Food Vendor', label: 'Caterer / Food Vendor' },
